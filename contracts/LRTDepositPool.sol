...
contract LRTDepositPool is ILRTDepositPool, LRTConfigRoleChecker, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function removeNodeDelegatorContractFromQueue(address nodeDelegatorAddress) public onlyLRTAdmin {
    ...
    (address[] memory assets, uint256[] memory assetBalances) = INodeDelegator(nodeDelegatorAddress).getAssetBalances();
  ...
  /// @param nodeDelegatorContracts Array of NodeDelegator contract addresses
  function removeManyNodeDelegatorContractsFromQueue(address[] calldata nodeDelegatorContracts) external onlyLRTAdmin {
    uint256 length = nodeDelegatorContracts.length;
  ...
  ) external onlyLRTManager onlySupportedAsset(fromAsset) onlySupportedAsset(toAsset) {
    // checks
