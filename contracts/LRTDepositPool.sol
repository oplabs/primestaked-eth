    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getAssetCurrentLimit(
        ...
        if (
            getTotalAssetDeposits(asset) > lrtConfig.depositLimitByAsset(asset)
        ) {
            return 0;
        }
        ...
        )
    {
        ...
        for (uint256 i; i < ndcsCount; ) {
            (uint256 _ndcAssets, uint256 _eigenAssets) = INodeDelegator(
    ...
    function getMintAmount(
        address asset,
        uint256 amount
    ) public view override returns (uint256 primeEthAmount) {
    ...
    function depositAsset(
        ...
        // checks
        uint256 primeETHAmount = _beforeDeposit(
            asset,
            depositAmount,
            minPrimeETH
        );
    ...
    function _beforeDeposit(
        ...
        uint256 depositAmount,
        uint256 minPrimeETH
    ) private view returns (uint256 primeETHAmount) {
    ...
    function transferAssetToNodeDelegator(
        ...
        address asset,
        uint256 amount
    ) external nonReentrant onlyLRTOperator onlySupportedAsset(asset) {
    ...
    ) external override nonReentrant onlyLRTOperator {
        address nodeDelegator = nodeDelegatorQueue[ndcIndex];
        UtilLib.checkNonZeroAddress(nodeDelegator);
    ...
    {
        // checks
        uint256 toAssetAmount = getSwapAssetReturnAmount(
            fromAsset,
            toAsset,
            fromAssetAmount
        );
        if (
            ...
            IERC20(toAsset).balanceOf(address(this)) < toAssetAmount
        ) {
    ...
    function getSwapAssetReturnAmount(
        ...
        return
            (lrtOracle.getAssetPrice(fromAsset) * fromAssetAmount) /
