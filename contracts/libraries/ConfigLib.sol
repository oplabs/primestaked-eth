...
library ConfigLib {
    ...
    ) internal returns (LRTConfig config) {
    ...
    function initialize(
        ...
        address primeETHAddress
    ) internal {
        // initialize new LRTConfig contract
        address stETH = block.chainid == 1
            ? Addresses.STETH_TOKEN
            : AddressesGoerli.STETH_TOKEN;
        address ethx = block.chainid == 1
            ? Addresses.ETHX_TOKEN
            : AddressesGoerli.ETHX_TOKEN;
    ...
    function upgrade(
        ...
        console.log(
            "Upgraded LRTConfig proxy %s to new implementation %s",
            proxyAddress,
            newImpl
        );
