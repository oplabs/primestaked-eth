...
library PrimeStakedETHLib {
    function deployImpl() internal returns (address implementation) {
        ...
        console.log(
            "PrimeStakedETH implementation deployed at: %s",
            implementation
        );
    }
    ...
    ) internal returns (PrimeStakedETH primeETH) {
    ...
    function upgrade(address newImpl) internal returns (PrimeStakedETH) {
        ...
        address proxyAddress = block.chainid == 1
            ? Addresses.PRIME_STAKED_ETH
            : AddressesGoerli.PRIME_STAKED_ETH;

        ProxyAdmin proxyAdmin = ProxyAdmin(proxyAdminAddress);

        proxyAdmin.upgrade(ITransparentUpgradeableProxy(proxyAddress), newImpl);
        console.log(
            "Upgraded PrimeStakedETH proxy %s to new implementation %s",
            proxyAddress,
            newImpl
        );
