...
library DepositPoolLib {
    function deployImpl() internal returns (address implementation) {
        ...
        console.log(
            "LRTDepositPool implementation deployed at: %s",
            implementation
        );
    }
    ...
    ) internal returns (LRTDepositPool depositPool) {
    ...
    function upgrade(address newImpl) internal returns (LRTDepositPool) {
        ...
        address proxyAddress = block.chainid == 1
            ? Addresses.LRT_DEPOSIT_POOL
            : AddressesGoerli.LRT_DEPOSIT_POOL;
        ...
        console.log(
            "Upgraded LRTDepositPool proxy %s to new implementation %s",
            proxyAddress,
            newImpl
        );
    ...
    function addNodeDelegator(address nodeDelegatorProxy) internal {
        address depositPoolAddress = block.chainid == 1
            ? Addresses.LRT_DEPOSIT_POOL
            : AddressesGoerli.LRT_DEPOSIT_POOL;
