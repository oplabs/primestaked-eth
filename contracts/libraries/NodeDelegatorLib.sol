...
library NodeDelegatorLib {
    function deployImpl() internal returns (address implementation) {
        ...
        console.log(
            "NodeDelegator implementation deployed at: %s",
            implementation
        );
    }
    ...
    function deployProxy(
        ...
        uint256 index
    ) internal returns (NodeDelegator nodeDelegator) {
        bytes32 salt = keccak256(
            abi.encodePacked("Prime-Staked-nodeDelegator", index)
        );
    ...
    function deployInit(
        ...
        address proxyAdminAddress = block.chainid == 1
            ? Addresses.PROXY_ADMIN
            : AddressesGoerli.PROXY_ADMIN;
        bytes32 salt = keccak256(
            abi.encodePacked("Prime-Staked-nodeDelegator", index)
        );

        address proxyFactoryAddress = block.chainid == 1
            ? Addresses.PROXY_FACTORY
            : AddressesGoerli.PROXY_FACTORY;
        address nodeDelegatorProxy = ProxyFactory(proxyFactoryAddress).create(
            nodeDelegatorImpl,
            proxyAdminAddress,
            salt
        );
        ...
        address lrtConfigProxy = block.chainid == 1
            ? Addresses.LRT_CONFIG
            : AddressesGoerli.LRT_CONFIG;
        nodeDelegator.initialize(lrtConfigProxy);

        console.log(
            "Native staking node delegator (proxy) deployed at: ",
            nodeDelegatorProxy
        );
    }
    ...
    function upgrade(
        ...
        address newImpl
    ) internal returns (NodeDelegator) {
        address proxyAdminAddress = block.chainid == 1
            ? Addresses.PROXY_ADMIN
            : AddressesGoerli.PROXY_ADMIN;
        ...
        console.log(
            "Upgraded NodeDelegator proxy %s to new implementation %s",
            proxyAddress,
            newImpl
        );
