...
library OraclesLib {
    ...
    ) internal returns (LRTOracle oracle) {
    ...
    function upgradeLRTOracle(address implementation) internal {
        ...
        proxyAdmin.upgrade(
            ITransparentUpgradeableProxy(Addresses.LRT_ORACLE),
            implementation
        );
    }
    ...
    function deployInitChainlinkOracle(
        ...
        proxy = proxyFactory.create(
            implContract,
            address(proxyAdmin),
            keccak256(abi.encodePacked("ChainlinkPriceOracleProxy"))
        );
    ...
    function deployChainlinkOracle() internal returns (address newImpl) {
        ...
        console.log(
            "ChainlinkPriceOracle implementation deployed at: %s",
            newImpl
        );
    }
    ...
    function upgradeChainlinkOracle(address newImpl) internal {
        ...
        proxyAdmin.upgrade(
            ITransparentUpgradeableProxy(Addresses.CHAINLINK_ORACLE_PROXY),
            newImpl
        );
    }

    function deployInitOETHOracle(
        ProxyAdmin proxyAdmin,
        ProxyFactory proxyFactory
    ) internal returns (address proxy) {
        ...
        address implContract = address(
            new OETHPriceOracle(Addresses.OETH_TOKEN)
        );
    ...
    function deployInitEthXPriceOracle(
        ...
        proxy = proxyFactory.create(
            implContract,
            address(proxyAdmin),
            keccak256(abi.encodePacked("EthXPriceOracleProxy"))
        );
        console.log("EthXPriceOracleProxy deployed at: %s", proxy);

        // Initialize the proxy
        address staderStakingPoolManager = block.chainid == 1
            ? Addresses.STADER_STAKING_POOL_MANAGER
            : AddressesGoerli.STADER_STAKING_POOL_MANAGER;
    ...
    function deployInitMEthPriceOracle() internal returns (address proxy) {
        ...
        address implContract = address(
            new MEthPriceOracle(Addresses.METH_TOKEN, Addresses.METH_STAKING)
        );
        ...
        proxy = proxyFactory.create(
            implContract,
            proxyAdmin,
            keccak256(abi.encodePacked("MEthPriceOracleProxy"))
        );
    ...
    function deployInitSfrxEthPriceOracle() internal returns (address proxy) {
        ...
        address implContract = address(
            new SfrxETHPriceOracle(
                Addresses.SFRXETH_TOKEN,
                Addresses.FRAX_DUAL_ORACLE
            )
        );
    ...
    function deployInitWETHOracle(
        ProxyAdmin proxyAdmin,
        ProxyFactory proxyFactory
    ) internal returns (address proxy) {
        address wethAddress = block.chainid == 1
            ? Addresses.WETH_TOKEN
            : AddressesGoerli.WETH_TOKEN;
        ...
        proxy = proxyFactory.create(
            implContract,
            address(proxyAdmin),
            keccak256(abi.encodePacked("WETHPriceOracleProxy"))
        );
