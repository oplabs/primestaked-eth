...
library OraclesLib {
  ...
  function deployInitOETHOracle(ProxyAdmin proxyAdmin, ProxyFactory proxyFactory) internal returns (address proxy) {
    ...
    proxy = proxyFactory.create(implContract, address(proxyAdmin), keccak256(abi.encodePacked("OETHPriceOracleProxy")));
  ...
  function deployInitEthXPriceOracle(
    ...
    proxy = proxyFactory.create(implContract, address(proxyAdmin), keccak256(abi.encodePacked("EthXPriceOracleProxy")));
  ...
  function deployInitWETHOracle(ProxyAdmin proxyAdmin, ProxyFactory proxyFactory) internal returns (address proxy) {
    ...
    proxy = proxyFactory.create(implContract, address(proxyAdmin), keccak256(abi.encodePacked("WETHPriceOracleProxy")));
