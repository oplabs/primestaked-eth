...
library AddAssetsLib {
    function getConfig() internal view returns (LRTConfig) {
        address configAddress = block.chainid == 1
            ? Addresses.LRT_CONFIG
            : AddressesGoerli.LRT_CONFIG;
        return LRTConfig(configAddress);
    }

    function getOracle() internal view returns (LRTOracle) {
        address oracleAddress = block.chainid == 1
            ? Addresses.LRT_ORACLE
            : AddressesGoerli.LRT_ORACLE;
    ...
    function addOETH() internal {
        configureAsset(
            LRTConstants.OETH_TOKEN,
            Addresses.OETH_TOKEN,
            Addresses.OETH_EIGEN_STRATEGY,
            Addresses.OETH_ORACLE_PROXY
        );
    ...
    function addMEth() internal {
        configureAsset(
            LRTConstants.M_ETH_TOKEN,
            Addresses.METH_TOKEN,
            Addresses.METH_EIGEN_STRATEGY,
    ...
    function addStETH() internal {
        ...
        ChainlinkPriceOracle chainlinkOracleProxy = ChainlinkPriceOracle(
            Addresses.CHAINLINK_ORACLE_PROXY
        );
        chainlinkOracleProxy.updatePriceFeedFor(
            Addresses.STETH_TOKEN,
            Addresses.STETH_ORACLE
        );

        LRTOracle lrtOracle = getOracle();
        lrtOracle.updatePriceOracleFor(
            Addresses.STETH_TOKEN,
            address(chainlinkOracleProxy)
        );
    ...
    function addRETH() internal {
        addAssetWithChainlinkOracle(
            LRTConstants.R_ETH_TOKEN,
            Addresses.RETH_TOKEN,
            Addresses.RETH_EIGEN_STRATEGY,
            Addresses.RETH_ORACLE
        );
    ...
    function addSwETH() internal {
        addAssetWithChainlinkOracle(
            LRTConstants.SWETH_TOKEN,
            Addresses.SWETH_TOKEN,
            Addresses.SWETH_EIGEN_STRATEGY,
            Addresses.SWETH_ORACLE
        );
    ...
    function addETHx() internal {
        ...
        lrtOracle.updatePriceOracleFor(
            Addresses.ETHX_TOKEN,
            Addresses.ETHX_ORACLE_PROXY
        );
    ...
    function addAssetWithChainlinkOracle(
        ...
        NodeDelegator(payable(Addresses.NODE_DELEGATOR))
            .maxApproveToEigenStrategyManager(asset);
    }
