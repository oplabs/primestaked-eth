    ...
    ReentrancyGuardUpgradeable
{
    ...
    function maxApproveToEigenStrategyManager(
        address asset
    ) external override onlySupportedAsset(asset) onlyLRTManager {
    ...
    ) external override whenNotPaused nonReentrant onlyLRTOperator {
    ...
    function transferBackToLRTDepositPool(
        ...
        onlyLRTManager
    {
        address lrtDepositPool = lrtConfig.getContract(
        ...
        returns (address[] memory assets, uint256[] memory assetBalances)
    {
        ...
        for (uint256 i = 0; i < assets.length; ) {
            (uint256 ndcAssets, uint256 eigenAssets) = getAssetBalance(
                assets[i]
            );
    ...
    function getAssetBalance(
        address asset
    ) public view override returns (uint256 ndcAssets, uint256 eigenAssets) {
        ...
        if (asset == WETH_TOKEN_ADDRESS) {
            ...
            address strategy = lrtConfig.assetStrategy(asset);
            if (strategy != address(0)) {
                eigenAssets = IStrategy(strategy).userUnderlyingView(
                    address(this)
                );
            }
        }
    ...
    ) external whenNotPaused nonReentrant onlyLRTOperator {
    ...
    function approveSSV() external onlyLRTManager {
        address SSV_TOKEN_ADDRESS = lrtConfig.getContract(
            LRTConstants.SSV_TOKEN
        );
        address SSV_NETWORK_ADDRESS = lrtConfig.getContract(
            LRTConstants.SSV_NETWORK
        );
    ...
    ) external onlyLRTOperator whenNotPaused {
        address SSV_NETWORK_ADDRESS = lrtConfig.getContract(
            LRTConstants.SSV_NETWORK
        );
