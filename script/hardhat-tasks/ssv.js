...
const getClusterInfo = async ({ ownerAddress, providerUrl, ssvNetwork, networkId, operatorids }) => {
  const operatorIds = operatorids.split(".").map((id) => parseInt(id));
  log(`Fetching cluster info for cluster owner ${ownerAddress} with operator ids: ${operatorIds}`);
