...
async function main() {
  ...
  const prETHFactory = await ethers.getContractFactory('PrimeStakedETH')
  const prETHToken = await upgrades.deployProxy(prETHFactory, [deployer.address, lrtConfig.target])
  console.log('prETH Token deployed at ', prETHToken.target)
